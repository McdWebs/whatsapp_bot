...
export class TwilioProvider extends BaseWhatsAppProvider {
  ...
  async sendInteractiveMessage(
    ...
    try {
      // For now, we'll send as a regular text message with formatted options
      // To use true interactive messages, you need to create Content Templates in Twilio Console
      // and use Content API with interactive components
      ...
      logger.info('Sending interactive message as text (fallback)', {
        to: normalizedTo,
        body: messageBody,
        hasButtons: !!buttons,
        hasList: !!listItems,
      ...
      logger.info('Twilio interactive message sent', {
        messageSid: message.sid,
        status: message.status,
      });
      ...
      logger.error('Twilio sendInteractiveMessage error', {
        to,
  ...
  parseWebhookPayload(payload: any): WhatsAppWebhookPayload {
    ...
    const interactivePayload = payload.ButtonText || payload.ListId;
    
    return {
      messageId: payload.MessageSid || payload.SmsSid || '',
      from: payload.From || payload.From || '',
      to: payload.To || payload.To || '',
      body: payload.Body || payload.ButtonText || payload.ListId || '',
      timestamp: payload.DateSent || new Date().toISOString(),
      type: payload.MessageType || (interactivePayload ? 'interactive' : 'text'),
      ...payload,
    };
  }
