...
const getProviderConfig = () => {
  if (isMockMode) {
    // Return mock config in mock mode
    return {
      twilio: {
        accountSid: 'AC' + 'mock_account_sid',
...
export const config: Config = {
  ...
  whatsapp: {
    ...
    ...getProviderConfig(),
    templates: {
      welcome: isMockMode ? 'mock_welcome' : getRequiredEnv('WHATSAPP_TEMPLATE_WELCOME'),
      reminder: isMockMode ? 'mock_reminder' : getRequiredEnv('WHATSAPP_TEMPLATE_REMINDER'),
      confirmation: isMockMode ? 'mock_confirmation' : getRequiredEnv('WHATSAPP_TEMPLATE_CONFIRMATION'),
      help: isMockMode ? 'mock_help' : getRequiredEnv('WHATSAPP_TEMPLATE_HELP'),
    },
  },
  admin: {
