...
export class SelectingReminderStateHandler implements StateHandler {
  ...
  async handle(context: StateContext, message: string): Promise<StateContext> {
    ...
    if (!reminderType) {
      await this.sendReminderTypePromptInteractive(context.phoneNumber);
      return context;
    }
    ...
    // If it's tefillin, ask for time offset with interactive buttons
    if (reminderType === 'tefillin') {
      await this.sendTefillinTimePromptInteractive(context.phoneNumber);
      return {
        ...newContext,
...
1. Tefillin reminder
2. Sunset times
3. Candle-lighting times (Shabbat)
4. Prayer times
5. Custom time reminder
...
Reply with the number or name.`;
  ...
  private async sendTefillinTimePromptInteractive(phoneNumber: string): Promise<void> {
    try {
      await whatsappMessageService.sendInteractiveMessage(
        phoneNumber,
        '⏰ בחר זמן תזכורת להנחת תפילין:',
        [
          { id: '20min', title: '20 דקות' },
          { id: '30min', title: '30 דקות' },
          { id: '1hour', title: 'שעה' },
        ]
      );
    } catch (error) {
      logger.error('Error sending tefillin time prompt', { phoneNumber, error });
    }
  }

  private async sendReminderTypePromptInteractive(phoneNumber: string): Promise<void> {
    try {
      await whatsappMessageService.sendInteractiveMessage(
        phoneNumber,
        'בחר סוג תזכורת:',
